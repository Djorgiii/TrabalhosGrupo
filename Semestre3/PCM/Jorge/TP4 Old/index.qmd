...
mapaVencedor = {
  ...
  const map = L.map(div).setView([39.6, -8], 7);
...
```{ojs}
//| title: "Visualização Sunburst"
sunburst = {
  if (!dadosOK) return md`A carregar visualização...`;
  ...
  const sunburstVotosNac = partidos.map(p => ({
    partido: p, votos: (resultados[p] || {})["Total"] || 0
  }));
  const total = sunburstVotosNac.reduce((acc, x) => acc + x.votos, 0);
  
  const grandes = sunburstVotosNac.filter(x => x.votos/total >= 0.02);
  const outros = sunburstVotosNac.filter(x => x.votos/total < 0.02);
  
  const data = {
    name: "Portugal",
    children: [
      ...grandes.map(p => ({
         name: p.partido, color: getColor(p.partido),
         children: distritos.map(d => ({ name: d, value: (resultados[p.partido]||{})[d]||0 }))
      })),
      {
         name: "Outros", color: "#ccc",
         children: distritos.map(d => ({
           name: d, value: outros.reduce((acc, p) => acc + ((resultados[p.partido]||{})[d]||0), 0)
         }))
      }
    ]
  };
  ...
  g.append("g").selectAll("path").data(root.descendants().slice(1)).join("path")
    .attr("fill", d => { while (d.depth > 1) d = d.parent; return d.data.color || "#ccc"; })
    .attr("fill-opacity", 0.8).attr("d", arc)
    .append("title").text(d => `${d.data.name}\n${Math.round(d.value).toLocaleString()}`);
    
  g.append("text").attr("text-anchor", "middle").text("Portugal").style("fill", "#666");
